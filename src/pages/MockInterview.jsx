...
import RoleBasedPage from "./RoleBased";
import Instructions from "./Instructions";

const MockInterview = ({ onBackToAI }) => {
  const [page, setPage] = useState("home");
  const [config, setConfig] = useState(null);

  // 🔥 Navigation Helpers (clean code)
  const goToHome = () => setPage("home");
  const goToPerformance = () => setPage("performance");
  const goToRole = () => setPage("role"); // ✅ IMPORTANT
  const goToInstructions = (data) => {
    setConfig(data);
  ...
  return (
    <div className="w-full overflow-x-hidden">
      ...
      {page === "role" && (
        <RoleBasedPage
          onBack={goToPerformance}
          onNext={goToInstructions}
        />
      )}
      ...
      {page === "instructions" && (
        <Instructions
          config={config}
          onBack={goToRole} // ✅ BACK FIXED HERE
        />
      )}
